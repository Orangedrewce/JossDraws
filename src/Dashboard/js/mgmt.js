(function () {
        ...
        // ============================================
        {
            ...
            const bEl = {
                ...
                canvas: document.getElementById('bannerPreviewCanvas'),
            ...
            // Default config values (mirrors WEBGL_CONFIG from webgl.js)
            const BANNER_DEFAULTS = {
                ...
                thickness: { base: 0.10, stretchMin: 0.8, stretchMax: 1.2, stretchSpeed: 1.3, stretchFrequency: 2.5 },
                wave: { mainSpeed: 1.0, mainFrequency: 3.0, mainAmplitude: 0.25, secondarySpeed: 1.8, secondaryFreq: 1.1, secondaryAmp: 0.1, horizontalSpeed: 0.7, horizontalFrequency: 2.0, horizontalAmount: 0.25, offsetBlend: 0.3 },
                twist: { enabled: true, period: 6.0, duration: 0.9, intensity: 0.5, randomSeed: 12.345 },
                appearance: { brightness: 1.125, plasticEffect: true, centerSoftness: 10.0, specularPower: 50.0, specularIntensity: 0.75, shadowStrength: 0.1, shadowWidth: 2.0, aaSharpness: 0.5, aaFallback: 0.001 },
                positioning: { verticalOffset: 0.205, bandCount: 5 },
                interaction: { hoverSlowdown: 0.1, smoothTime: 0.25 },
                performance: { supersampleDesktop: 2.5, supersampleMobile: 1.0, mobileBreakpoint: 768, respectDPR: true, pauseWhenHidden: true, maxDeltaTime: 0.05, debugMode: false }
            };

            // Param→control mapping: { group.key : { type: 'range'|'checkbox', id: string } }
            const PARAM_MAP = [
            ...
            // Read / write nested paths
            function getPath(obj, path) {
                const keys = path.split('.');
                let cur = obj;
                for (const k of keys) { cur = cur[k]; if (cur === undefined) return undefined; }
                return cur;
            }
            ...
            // Live config (starts as clone of defaults)
            let liveConfig = deepClone(BANNER_DEFAULTS);

            // ---- Control sync (range ↔ number) ----
            function syncControls() {
                for (const p of PARAM_MAP) {
                    ...
                    if (p.type === 'checkbox') {
                        ...
                        el.value = val;
                        // Sync companion number input
                        const numEl = document.getElementById(p.id + '-num');
            ...
            function readControlValue(p) {
                ...
                if (p.type === 'checkbox') return el.checked;
                return parseFloat(el.value);
            }

            function readAllControls() {
                for (const p of PARAM_MAP) {
                    const val = readControlValue(p);
                    if (val !== undefined) setPath(liveConfig, p.path, val);
                }
            }

            // ---- Attach input listeners (bidirectional slider ↔ number) ----
            ...
            const fmtVec3 = (c) => `vec3(${fmtF(c.r)}, ${fmtF(c.g)}, ${fmtF(c.b)})`;

            function buildFragmentShader(cfg) {
                const bandMax = cfg.positioning.bandCount - 1;
                const twistEnabled = cfg.twist.enabled;
                const maxWaveAbs = cfg.wave.mainAmplitude + cfg.wave.secondaryAmp + (cfg.wave.horizontalAmount * Math.abs(cfg.wave.offsetBlend));
                const maxRibbonHalfHeight = maxWaveAbs + (cfg.thickness.base * cfg.thickness.stretchMax * cfg.positioning.bandCount) + cfg.appearance.aaFallback;
            ...
            function compileShaderSrc(gl, src, type) {
                ...
                if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
                    console.error('[BannerPreview] Shader compile error:', gl.getShaderInfoLog(shader));
            ...
            function createPreviewProgram(gl, fragSrc) {
                ...
                const fs = compileShaderSrc(gl, fragSrc, gl.FRAGMENT_SHADER);
                ...
                if (!gl.getProgramParameter(prog, gl.LINK_STATUS)) {
                    console.error('[BannerPreview] Link error:', gl.getProgramInfoLog(prog));
            ...
            function initPreview() {
                const canvas = bEl.canvas;
                if (!canvas) return false;
                const gl = canvas.getContext('webgl', { alpha: false, antialias: false }) ||
                           canvas.getContext('experimental-webgl', { alpha: false, antialias: false });
                if (!gl) { console.warn('[BannerPreview] WebGL not supported.'); return false; }
            ...
            function rebuildShader() {
                ...
                if (previewState.program) {
                    gl.deleteProgram(previewState.program);
                    previewState.program = null;
                }

                const fragSrc = buildFragmentShader(liveConfig);
                const prog = createPreviewProgram(gl, fragSrc);
            ...
            function renderPreviewFrame(now) {
                ...
                const gl = previewState.gl;
                if (!gl || !previewState.program) return;
            ...
            function rebuildPreview() {
                ...
                const wasRunning = previewState.running;
                stopPreview();
                if (rebuildShader() && wasRunning) startPreview();
            }

            // ---- Message helper ----
            function showBannerMsg(msg, isError) {
                if (!bEl.message) return;
                bEl.message.textContent = msg;
                bEl.message.className = 'gallery-msg' + (isError ? ' gallery-msg-error' : ' gallery-msg-success');
                bEl.message.classList.remove('is-hidden');
                setTimeout(() => bEl.message.classList.add('is-hidden'), 4000);
            }

            // Build publishable config object (no colors — those stay in source)
            function buildPublishConfig() {
                readAllControls();
                return {
                    thickness: { ...liveConfig.thickness },
                    wave: { ...liveConfig.wave },
                    twist: { ...liveConfig.twist },
                    appearance: { ...liveConfig.appearance },
                    positioning: { ...liveConfig.positioning },
                    interaction: { ...liveConfig.interaction },
                    performance: { ...liveConfig.performance }
                };
            }

            // ---- Load saved config from Supabase ----
            let bannerConfigLoaded = false;
            async function loadBannerConfig() {
                if (bannerConfigLoaded || !db || !adminCode) return;
                try {
                    ...
                    bannerConfigLoaded = true;
                    if (data.config && typeof data.config === 'object') {
                        // Merge saved config over defaults (preserving colors from defaults)
                        ...
                        for (const group of ['thickness', 'wave', 'twist', 'appearance', 'positioning', 'interaction', 'performance']) {
                            if (saved[group] && typeof saved[group] === 'object') {
                                for (const key in saved[group]) {
                                    if (liveConfig[group] && key in liveConfig[group]) {
                                        liveConfig[group][key] = saved[group][key];
                                    }
                                }
                            }
                        }
                        syncControls();
                        rebuildPreview();
                        Trace.log('BANNER_CONFIG_LOADED', { keys: Object.keys(saved) });
                    } else {
                        Trace.log('BANNER_CONFIG_EMPTY');
                    }
                } catch (err) {
                    Trace.log('BANNER_CONFIG_LOAD_ERR', { message: err.message });
                }
            }
            ...
            if (bEl.resetBtn) {
                bEl.resetBtn.addEventListener('click', () => {
                    ...
                    syncControls();
                    rebuildPreview();
                    showBannerMsg('Parameters reset to defaults. Click Publish to save.', false);
                    Trace.log('BANNER_PARAMS_RESET');
                });
            }

            // ---- Publish to Supabase (Apply) ----
            if (bEl.exportBtn) {
                bEl.exportBtn.addEventListener('click', async () => {
                    if (!db || !adminCode) {
                        showBannerMsg('Not authenticated.', true);
                        return;
                    }
                    ...
                    try {
                        ...
                        if (!data || !data.success) {
                            ...
                            showBannerMsg(data?.error || 'Save failed', true);
                            return;
                        }
                        showBannerMsg('Banner config published! Visitors will see changes on refresh.', false);
                        Trace.log('BANNER_CONFIG_PUBLISHED');
                        ...
                        showBannerMsg('Error: ' + err.message, true);
                    } finally {
            ...
            let bannerPreviewInited = false;
            if (bEl.section) {
                bEl.section.addEventListener('toggle', () => {
                    if (bEl.section.open) {
                        if (!bannerPreviewInited) {
                            bannerPreviewInited = true;
                            // Load saved config first, then init preview
                            loadBannerConfig().then(() => {
                                syncControls();
                                const ok = initPreview();
                                if (ok) {
                                    if (bEl.overlay) bEl.overlay.classList.add('is-hidden');
                                    startPreview();
                                    Trace.log('BANNER_PREVIEW_INIT_OK');
                                } else {
                                    if (bEl.overlay) bEl.overlay.innerHTML = '<span style="color:var(--color-danger);font-size:0.82rem;">WebGL preview unavailable</span>';
                                    Trace.log('BANNER_PREVIEW_INIT_FAIL');
                                }
                            });
                        } else {
                            startPreview();
